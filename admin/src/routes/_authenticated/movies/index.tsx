...
import { getMovieColumns } from "@/components/content/columns";
...
const CONTENT_TYPE = "MOVIE" as const;
const PAGE_SIZE = 20;
...
function MoviesPage() {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      void queryClient.invalidateQueries({ queryKey: ["content", CONTENT_TYPE] });
  ...
  const unpublishMutation = useMutation({
    ...
    onError: () => {
      toast.error("Failed to unpublish movie");
    },
  });

  const handleEdit = (content: Content) => {
    void navigate({ to: `/movies/${content.id}` as string });
  };
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <Button
            ...
            onClick={() => void navigate({ to: "/movies/new" as string })}
          >
