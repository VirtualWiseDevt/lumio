...
export const documentaryEditRoute = createRoute({
  getParentRoute: () => authenticatedRoute,
  path: "/documentaries/$docId",
  component: DocumentaryEditPage,
});

function DocumentaryEditPage() {
  const navigate = useNavigate();
  const params = useParams({ strict: false }) as { docId?: string };
  const docId = params.docId ?? "";

  const { data: content, isLoading, error } = useQuery({
    queryKey: ["content", docId],
  ...
  if (isLoading) {
    return (
      <PageContainer title="Edit Documentary">
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-md border border-destructive/50 bg-destructive/10 p-6 text-center">
            <p className="text-destructive">
              Documentary not found or failed to load.
            </p>
          </div>
        </div>
      </PageContainer>
